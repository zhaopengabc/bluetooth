...
{  int a=0;
  ...
  //while (SPI_I2S_GetFlagStatus(SPI3, SPI_I2S_FLAG_RXNE) == RESET);
    while(!(SPI3->SR & SPI_I2S_FLAG_RXNE)){a++;};

  /* Return the byte read from the SPI3 bus */
  // return SPI_I2S_ReceiveData(SPI3);
//   printf("SPI:%x.%d_",SPI3->DR,a);
   return SPI3->DR;
}
...
//  GPIO_PinRemapConfig(GPIO_Remap_SPI3, ENABLE );
  ...
  /**
  *	SPI3_SCK -> PA5 , SPI3_MISO -> PA6 , SPI3_MOSI ->	PA7
  */		
...
void MSD0_SPIHighSpeed(uint8_t b_high)
{
  ...
  if(b_high == 0)
  {
	 SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_256;//SPI_BaudRatePrescaler_256;
  }
  else
  {
...
int MSD0_Init(void)
{
	...
	for(retry=0; retry<0x7F; retry++) //0xFFF
	{
//		printf("OK ON!\r\n");
...
//////////////////////test//////////////////////////////////////
		for(retry=0; retry<0xFFF; retry++)
		{
		 ...
		 //r1 = MSD0_send_command(ACMD41, 0x00000000, 0);	/* should be return 0x00 */
		 //r1 = MSD0_send_command(ACMD41, 0x00200000, 0);
		  	r1 = MSD0_send_command(ACMD41,   0x40000000, 0);
//  			printf("A=%d",r1);
			if(r1 == 0x00)//问题1；
			//if(r1 == 0x00)
			{
...
//      do
//      {
//       r1 = MSD0_send_command(CMD55, 0, 0);                        //??CMD55,???0x01;????
//											if(r1 != 0x01)
//											{        
//												 return r1;  
//											}
//											for(i=0;i<0x800;i++);
//											r1 = MSD0_send_command(ACMD41,0x40000000,0x00);
//											retry++;
