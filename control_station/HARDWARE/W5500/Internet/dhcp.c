...
uint8 EXTERN_DHCPBUF[1024] __attribute__((aligned(4))); //4个字节对齐
  ...
  unsigned char b=0;
	...
	switch(dhcpret)
	{
		...
		case DHCP_RET_UPDATE:														 /*成功获取到IP地址*/ 
		  dhcp_ok=1;                  
//			set_w5500_ip();                                /*将获取到的IP地址写入W5500寄存器*/ 下步函数中初始化IP
		  printf("2.DHCP:已从DHCP服务器成功获得IP地址\r\n");
      b=1;
	    break;
		
		case DHCP_RET_CONFLICT:                          /*IP地址获取冲突*/ 
			printf("2.DHCP:从DHCP获取IP地址失败\r\n");
      dhcp_state = STATE_DHCP_READY; 
      printf("2.DHCP:重试中\r\n");
      dhcp_ok=0; 
			break;     
    ...
    if(b==1)
		{
		 printf("1.DHCP成功:=%d\r\n",i); 
	   printf("-----DHCP更新IP地址-------\r\n"); 
		 W5500_Init();//更新IP地址；do_dhcp中已经更新地址了
		 break;
		}
	}
	if(i>=10)
	{
		 printf("2.DHCP失败=%d,解析失败！\r\n",i); 
	}
